...
def load_decision_data():
    ...
    relevant_stock_file = Path(PROJECT_PATH) / 'data/basic/relevant_stock.csv'
    ...
    try:
        ...
        if not relevant_stock_df.empty:
            df = df.merge(relevant_stock_df, how='left', on='symbol')
        return df
...
def display_detailed_data(filtered_df):
    ...
    column_config = {
        ...
        "relevant_stock": st.column_config.Column(
            "相关股票",
            help="relevant_stock",
            width="small"
        ),
