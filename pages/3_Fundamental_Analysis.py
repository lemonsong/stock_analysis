...
industry_file = Path(PROJECT_PATH) / 'data_ak_industry' / 'stock_industry.csv'
    ...
    try:
        ...
        if industry_file.exists():
            try:
                ...
                if industry_col:
                    df = pd.merge(df, df_ind[['symbol', industry_col]], on='symbol', how='left')
...
def get_stock_list(_df):
    """获取股票列表（缓存）"""
    cols = ['symbol', 'SECURITY_NAME_ABBR']
    if 'industry' in _df.columns:
        cols.append('industry')
    return _df[cols].drop_duplicates()
    ...
    if file_path.exists():
        try:
            ...
            if 'date' in df.columns:
                ...
                if 'index' in df.columns: # 假设索引是日期
                     try:
...
def setup_sidebar(df):
    ...
    elif selection_method == "📝 列表输入":
        st.sidebar.caption("输入股票代码，用逗号分隔")
        input_text = st.sidebar.text_area("股票代码列表", "000001, 600519", key="list_input")
        if input_text:
...
def render_comprehensive_tab(df, selected_symbols, stock_names):
    ...
    if 'fiscal_year' in df.columns:
        ...
        if not baseline_data.empty:
            ...
            try:
                st.dataframe(display_df.style.background_gradient(cmap='RdYlGn'), use_container_width=True)
            except:
